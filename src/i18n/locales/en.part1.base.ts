const enPart1Base = {
  // Common
  common: {
    loading: "Loading…",
    saving: "Saving…",
    creating: "Creating…",
    deleting: "Deleting…",
    save: "Save",
    create: "Create",
    cancel: "Cancel",
    delete: "Delete",
    remove: "Remove",
    removeOut: "Remove",
    close: "Close",
    restore: "Restore",
    confirm: "Confirm",
    ok: "OK",
    edit: "Edit",
    refresh: "Refresh",
    retry: "Retry",
    dismiss: "Dismiss",
    later: "Later",
    yes: "Yes",
    no: "No",
    done: "Done",
    error: "Error",
    success: "Success",
    warning: "Warning",
    info: "Info",
    search: "Search",
    clear: "Clear",
    all: "All",
    none: "None",
    default: "Default",
    thinking: "Thinking",
    notFound: "Not found",
    truncated: "Truncated",
    name: "Name",
    terminal: "Terminal",
    toggleTerminalPanel: "Toggle terminal panel",
  },

  // App
  app: {
    title: "ccgui",
    subtitle: "Orchestrate agents across your local projects.",
  },

  searchPalette: {
    placeholder: "Search files, kanban, threads, messages, skills, commands...",
    placeholderFiltered: "Search {{content}}...",
    inputAria: "Search input",
    matchOptions: "Match options",
    matchCaseLabel: "Match case",
    matchCaseSymbol: "Aa",
    wholeWordLabel: "Whole word",
    wholeWordSymbol: "W",
    scope: "Scope",
    content: "Content",
    current: "Current",
    global: "Global",
    currentWorkspace: "Current workspace",
    allWorkspaces: "All workspaces",
    noResults: "No results",
    noResultsHint:
      "Try keywords from file path, task title, thread title, message text, skill name, or command name.",
    navigate: "Navigate",
    open: "Open",
    close: "Close",
    projectTag: "Project",
    typeTag: "Type",
    sourceTag: "Source",
    locationTag: "Location",
    typeFile: "File",
    typeContent: "Content",
    typeKanban: "Kanban",
    typeThread: "Thread",
    typeMessage: "Message",
    typeHistory: "History",
    typeSkill: "Skill",
    typeCommand: "Command",
    sourceFiles: "File index",
    sourceContent: "File content",
    sourceKanban: "Kanban data",
    sourceThreads: "Thread titles",
    sourceMessages: "Message content",
    sourceHistory: "Input history",
    sourceSkills: "Skills list",
    sourceCommands: "Commands list",
    contentAll: "All",
    contentFiles: "Files",
    contentKanban: "Kanban",
    contentThreads: "Threads",
    contentMessages: "Messages",
    contentHistory: "History",
    contentSkills: "Skills",
    contentCommands: "Commands",
    contentFileContent: "File content",
    contentMinLengthTitle: "Keep typing to search file content",
    contentMinLengthHint: "File-content search needs at least {{count}} characters to avoid scanning the workspace on very short input.",
    contentSearching: "Searching file content...",
    contentMoreAvailable: "Scroll for more file matches",
    contentDegraded: "File content search returned partial results",
    contentDegradedWithReason: "File content search returned partial results: {{reason}}",
  },

  // TabBar
  tabbar: {
    projects: "Projects",
    codex: "Codex",
    spec: "Spec",
    git: "Git",
    log: "Log",
    primaryNavigation: "Primary navigation",
    workspaceNavigation: "Workspace navigation",
  },

  layout: {
    resizeGitHistoryPanel: "Resize git history panel",
    resizeSidebar: "Resize sidebar",
    resizeRightPanel: "Resize right panel",
    resizeEditorSplit: "Resize editor split",
    resizePlanPanel: "Resize plan panel",
    resizeRuntimeConsole: "Resize run console",
    resizeProjects: "Resize projects",
  },
};

export default enPart1Base;
