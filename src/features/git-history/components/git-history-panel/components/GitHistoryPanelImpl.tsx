...
export const GitHistoryPanel = memo(function GitHistoryPanel({
  ...
  const [detailsListView, setDetailsListView] = useState<"flat" | "tree">("tree");
  ...
  const [detailsSplitConnectorTop, setDetailsSplitConnectorTop] = useState<number | null>(null);
  ...
  const detailsFileTreeItems = useMemo(() => {
    if (!details) {
      return [];
    }
    if (detailsListView === "flat") {
  ...
  }, [details, detailsListView, fileTreeItems]);
  ...
  useEffect(() => {
    ...
    if (!details || !detailsBody || !mainGrid || !desktopSplitLayout) {
      setDetailsSplitConnectorTop(null);
      return undefined;
    }

    const updateConnectorTop = () => {
      const detailsBodyRect = detailsBody.getBoundingClientRect();
      const mainGridRect = mainGrid.getBoundingClientRect();
      const fileListHeight = Math.max(
        140,
        (detailsBody.clientHeight * detailsSplitRatio) / 100,
      );
      const nextTop = Math.round(detailsBodyRect.top - mainGridRect.top + fileListHeight);
      setDetailsSplitConnectorTop((currentTop) =>
