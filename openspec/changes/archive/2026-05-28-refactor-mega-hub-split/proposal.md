...
- **Task state**: 16/16 checked; status = Completed / pending verify-archive.
- **Code/document evidence**: `app-shell-parts/*`、`lazyViews.tsx`、`renderAppShell.tsx`、SpecHub/workspace sections 已形成拆分后 shell composition surface。
- **Next action**: 归档前确认 typecheck、runtime contract checker 与 key app-shell focused tests。
